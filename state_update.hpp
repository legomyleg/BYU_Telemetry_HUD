...
inline void update_vertical_velocity(float da, float dt_s, float &vert_velocity) {
    vert_velocity = da / dt_s;
}

inline void update_velocity(SensorData &s, float dt_s, Vec3 &velocity) {
...
inline void UpdateState(HudApp& app, SampleBuffer& samples, SerialPort& serial) {
